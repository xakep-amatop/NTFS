...
/** Insert item in orderly list
	\param
		...
		Compare	pointer to function for compare items in list,
				...
				in this function
*/
__declspec(dllimport) void __cdecl AddItemList(struct List *i, struct List **start, struct List **last, int (__cdecl *comp)(void*, void*));
...
*/
__declspec(dllimport) struct List * __cdecl FindItemList(struct List *start, void * n, comp_item Compare);
...
*/
__declspec(dllimport) void __cdecl FreeList(struct List **start, free_data fd);
